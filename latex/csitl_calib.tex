...
\title{Calibration of the CryspLight optical model on \csitl{}:\\
the absorption/scattering split of the measured extinction}
\author{J.J.~G\'omez-Cadenas \and Claude (Anthropic)}
\date{July 13, 2026 \\[2mm] \small Version 0.1 --- calibration note (phase 1)}
...
``absorption length'' would give. All numbers regenerate from
\texttt{scripts/csitl\_calib.jl} + \texttt{runs/csitl\_calib.toml}.
\end{abstract}
...
\begin{itemize}
  \item \texttt{scripts/csitl\_calib.jl} + \texttt{runs/csitl\_calib.toml} --- the
    campaign (grid, cases, targets);
  \item \texttt{data/csitl.toml} --- the calibrated parameters, provenance in comments;
  \item \texttt{runs/point\_csitl\_teflon.toml} --- the CRYSP-geometry application;
  \item \texttt{output/csitl\_calib/summary.csv} --- grid results;
