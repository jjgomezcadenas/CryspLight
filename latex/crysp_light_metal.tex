...
exponential time profile ($\tau \simeq 800$~ns for \csi{} at 77~K, $\simeq 1770$~ns for \bgo{}
...
\end{equation}
with $\tau = 800$~ns (\csi{}, 77~K) or $\tau = 1768$~ns (\bgo{}, 195~K). The photon then
...
\begin{table}[htb]
  \centering
  \caption{Crystal bulk and scintillation parameters (the content of
  \texttt{data/csi.toml} and \texttt{data/bgo.toml}). $n$ is quoted at the emission
  peak. All values are now \emph{calibrated} rather than assumed --- the full provenance
  chains (CRYSP measurement~\cite{soleti2024} for cold \csi{}; Ding--Liu, Gironnet and
  Mao--Zhang--Zhu for \bgo{}; the Brose/Knyazev CsI(Tl) exercise that fixed the
  methodology) are documented in the companion calibration note
  \texttt{csitl\_calib.tex}. The bulk is specified as the measured extinction
  $\Lambda_{\rm ext}$ split into absorption and elastic scattering,
  $1/\Lambda_{\rm ext} = 1/\Lambda_{\rm abs} + 1/\Lambda_{\rm R}$, with scattering
  fraction $f = 0.9$ for both (a choice on a degeneracy line for \csi{}, unmeasured for
  \bgo{} --- brackets in the calibration note).}
  \label{tab:crystals}
  \begin{tabular}{lcc}
    \toprule
    ...
    Light yield $Y$ (photons/MeV)       & $1.0\times10^5$ & $1.4\times10^4$ \\
    Decay time $\tau$ (ns)              & 800 ($\simeq$810 at 104~K) & 1768 \\
    Extinction $\Lambda_{\rm ext}$ (m)  & 0.30 (calibrated) & 2 (bound $\gtrsim$1.5--2) \\
    $\Rightarrow \Lambda_{\rm abs}$ / $\Lambda_{\rm R}$ (m) & 3.0 / 0.33 & 20 / 2.2 \\
    CRYSP light collection (PDE$=$1)    & 83.0\% [62--83] & 88.0\% [84.5--90] \\
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{Implementation status (built).} The port exists as a single
KernelAbstractions kernel (\texttt{src/kernel.jl}) --- one work-item per photon,
functional (isbits) Philox state, per-photon output records (status, sensor index,
time, bounces) instead of device-side atomics, reduced to the standard accumulator on
the host. The random-draw order mirrors the reference CPU path draw-for-draw, so on the
CPU backend the kernel is \emph{bit-identical} to the validated reference (enforced in
the test suite, all processes active); on Metal, transcendental ulp differences flip
only a few photons per million (statistical agreement enforced). Backend selection
follows the array type (\texttt{Array} $\to$ threaded CPU,
\texttt{Metal.MtlArray} $\to$ GPU). Measured throughput (M-series,
\texttt{scripts/bench\_metal.jl}, calibrated \csi{} configuration, $4\times10^6$
photons): \textbf{33~Mphotons/s end-to-end}, $6.2\times$ the 8-thread reference path
--- $\sim$1200 full 511~keV \csi{} events per second, in line with the estimate above.
One Metal-specific constraint worth recording: kernel arguments are capped at 31 buffer
bindings and every scalar counts, so parameters travel as the isbits configuration
structs rather than unpacked scalars.
...
($\lambda=350$~nm, $n=1.95$, $\tau=800$~ns, $Y=10^5$/MeV), \bgo{} ($\lambda=480$~nm,
$n=2.15$, $\tau=1768$~ns, $Y=1.4\times10^4$/MeV), grease $n_g=1.45$, Teflon
...
\begin{enumerate}
  \item \textbf{v0 --- CPU reference} (Julia, KernelAbstractions CPU backend): full physics
    ...
    complete.
  \item \textbf{v1 --- Metal port (built)}: same kernel on MtlArrays; bit-level agreement
    with v0 on identical Philox streams (achieved on the CPU backend; few-per-million
    statistical agreement on Metal); measured 33~Mphotons/s, $6.2\times$ the threaded
    CPU path.
